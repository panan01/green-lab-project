...
# --- Clases de bloques de construcción (sin cambios, ya que no están en la ruta crítica de rendimiento) ---
...
# --- Clases de polígonos optimizadas ---

class Rectangle:
    """
    A geometric rectangle on a 2D surface.
    G6 & G9: This class is simplified to not inherit from Polygon. It stores side
    lengths directly as numbers, avoiding the overhead of creating Side, Angle,
    and list objects. This significantly reduces object creation time and memory usage.
    ...
    def __init__(self, short_side_length: float, long_side_length: float) -> None:
        if not (
            isinstance(short_side_length, (int, float))
            and isinstance(long_side_length, (int, float))
            and short_side_length > 0
            and long_side_length > 0
        ):
    ...
    def perimeter(self) -> float:
        # G9: Calculation is direct, avoiding method calls to child objects.
...
class Square(Rectangle):
    ...
    # G6: Redundant 'perimeter' and 'area' methods removed. They are correctly
    # inherited from the optimized Rectangle class, so redefining them to call
...
def main(size: str) -> None:
    ...
    if size == "small":
        ...
        # La funcionalidad a pequeña escala permanece sin cambios.
        square = Square(10)
        ...
        # G6 & G7: Avoid creating and storing a large list of objects. A generator
        # expression is used to create objects one by one, calculate the area,
        # and sum them up in a single, memory-efficient pass.
        total_area = sum(Square(i * 0.01).area() for i in range(1, num_objects + 1))
        ...
        # G1, G6, G7: To avoid iterating over the objects twice (once for area,
        # once for perimeter), a single loop is used. This avoids storing the
        # large list of objects (G6) and processes both calculations in one pass (G1, G7).
        total_area = 0.0
        total_perimeter = 0.0
        for i in range(1, num_objects + 1):
            rect = Rectangle(short_side_length=i * 0.01, long_side_length=i * 0.02)
