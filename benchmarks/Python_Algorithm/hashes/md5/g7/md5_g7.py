...
def preprocess(message: bytes) -> bytes:
    ...
    True
    """
    # G7: Generate all bit strings at once and join them in a bulk operation.
    # This is more efficient than concatenating one by one in a loop.
    bit_string = b"".join(format(char, "08b").encode("utf-8") for char in message)
    
    start_len_val = len(bit_string)
    start_len = format(start_len_val, "064b").encode("utf-8")

    # Pad bit_string to a multiple of 512 chars
    bit_string += b"1"
    
    # G7: Calculate the number of padding zeros needed and add them in a single bulk operation.
    current_len = len(bit_string)
    padding_zeros_needed = (448 - (current_len % 512) + 512) % 512
    bit_string += b"0" * padding_zeros_needed
...
def get_block_words(bit_string: bytes) -> Generator[list[int], None, None]:
    ...
    for pos in range(0, len(bit_string), 512):
        ...
        # G7: Use a list comprehension to generate the list of words in a single, optimized operation
        # instead of appending one by one in a loop.
...
def not_32(i: int) -> int:
    ...
    # G7: Replace character-by-character string manipulation with a single, fast
    # bitwise bulk operation. This avoids the overhead of converting to/from strings
    # and handling individual characters.
    return i ^ 0xFFFFFFFF
