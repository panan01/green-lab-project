...
def to_little_endian(string_32: bytes) -> bytes:
    ...
    # G3 Optimization: Loop unrolling.
    # The original loop iterated four times. Unrolling it into a single
    # expression avoids the overhead of the loop structure.
...
def reformat_hex(i: int) -> bytes:
    ...
    # G3 Optimization: Loop unrolling.
    # Similar to to_little_endian, this fixed 4-iteration loop is unrolled
    # to eliminate loop overhead for a small performance gain.
    return (hex_rep[6:8] + hex_rep[4:6] + hex_rep[2:4] + hex_rep[0:2]).encode("utf-8")
...
def preprocess(message: bytes) -> bytes:
    ...
    bit_string += b"1"
    # G3 Optimization: Replace padding loop with a direct calculation.
    # Instead of looping and appending "0" one by one while checking the
    # length, we calculate the required number of zeros and append them
    # all at once. This avoids the overhead of the loop condition check.
    num_zeros = (448 - (len(bit_string) % 512) + 512) % 512
    bit_string += b"0" * num_zeros
...
def get_block_words(bit_string: bytes) -> Generator[list[int], None, None]:
    ...
    for pos in range(0, len(bit_string), 512):
        ...
        # Using a list comprehension can be slightly faster than a for-loop with .append()
        block_words = [int(to_little_endian(block[i : i + 32]), 2) for i in range(0, 512, 32)]
        yield block_words
...
def not_32(i: int) -> int:
    ...
    # G3 Optimization: The original implementation used a loop over a string
    # representation of the number, which is highly inefficient.
    # Replacing it with a single, fast bitwise XOR operation eliminates the
    # loop and the expensive string conversions entirely.
    return i ^ 0xFFFFFFFF
...
def md5_me(message: bytes) -> bytes:
    ...
    for block_words in get_block_words(bit_string):
        ...
        # G3 Optimization: Loop Unswitching.
        # The main loop of 64 iterations is split into four separate loops
        # of 16 iterations each. This technique avoids the conditional
        # `if/elif/else` check inside every single iteration. By removing this
        # repeated branching, we improve instruction pipeline efficiency
        # and reduce the risk of branch prediction misses.
