...
def lower_upper_decomposition(table: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    ...
    # G5 Optimization: Propagate the precision from the input matrix.
    # If the input is integer-based, it's converted to the default float
    # to prevent errors during division and ensure doctests pass. For
    # performance-critical code, the input should already be a float type.
    if np.issubdtype(table.dtype, np.integer):
        table = table.astype(np.float64)

    # By using table.dtype, we allow computations with lower precision (e.g., float32)
    # if the input matrix has that type, saving memory and potentially speeding up
    # the process.
    lower = np.zeros((rows, columns), dtype=table.dtype)
    upper = np.zeros((rows, columns), dtype=table.dtype)
    ...
    for i in range(columns):
        for j in range(i):
            total = np.sum(lower[i, :j] * upper[:j, j])
            if upper[j][j] == 0:
...
def main(size_preset: str) -> None:
    ...
    # G5 Optimization: Generate the matrix with single-precision floats (float32)
    # instead of the default double-precision (float64). This reduces memory usage
    # by half and can speed up computations, as high precision is not required for
    # this demonstration.
