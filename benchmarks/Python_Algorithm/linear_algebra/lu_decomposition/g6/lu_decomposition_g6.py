...
def lower_upper_decomposition(table: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    ...
    # G6 Optimization: Perform decomposition in-place on a copy of the table.
    # This avoids creating two separate zero-filled matrices (lower and upper),
    # reducing auxiliary memory from 2*n^2 to n^2. The combined lu_matrix
    # stores U in its upper triangle and L (excluding the identity diagonal)
    # in its strictly lower triangle.
    lu_matrix = table.astype(np.float64, copy=True)

    for i in range(rows):
        # Calculate the elements of L for the i-th row
        for j in range(i):
            dot_product = np.dot(lu_matrix[i, :j], lu_matrix[:j, j])
            if lu_matrix[j, j] == 0:
                raise ArithmeticError("No LU decomposition exists")
            lu_matrix[i, j] = (lu_matrix[i, j] - dot_product) / lu_matrix[j, j]

        # Calculate the elements of U for the i-th row
        for j in range(i, rows):
            dot_product = np.dot(lu_matrix[i, :i], lu_matrix[:i, j])
            lu_matrix[i, j] -= dot_product

    # Extract the lower and upper matrices from the combined lu_matrix.
    # The diagonal of the lower matrix consists of 1s (Doolittle's method).
    lower = np.tril(lu_matrix, k=-1) + np.identity(rows)
