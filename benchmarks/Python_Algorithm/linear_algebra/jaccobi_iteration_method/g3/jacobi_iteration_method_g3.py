...
def jacobi_iteration_method(
    ...
    # --- G3 Optimization Start ---
    # Move loop-invariant computations out of the loop to avoid redundant work.
    # 1. Pre-flatten the constant matrix. This operation is the same in every iteration.
    b_flat = constant_matrix.flatten()
    # 2. Pre-compute the reciprocal of the diagonal elements.
    #    This allows us to replace division with multiplication inside the loop,
    #    which is generally a faster operation.
    D_inv = 1.0 / D
    # --- G3 Optimization End ---

    # The iteration formula is x_k+1 = D^-1 * (b - R * x_k)
    for _ in range(iterations):
        # The update formula now uses the pre-computed, loop-invariant values.
        x = (b_flat - np.dot(R, x)) * D_inv
