...
def jacobi_iteration_method(
    ...
    # Extract the diagonal of the coefficient matrix
    D = np.diag(coefficient_matrix)

    # G6 Optimization: Avoid creating and storing the large intermediate matrix R.
    # The original code created R = coefficient_matrix - np.diag(D).
    # Instead, we compute the product R @ x directly inside the loop.

    # The iteration formula is x_k+1 = D^-1 * (b - R * x_k)
    for _ in range(iterations):
        # The term np.dot(R, x) can be computed as np.dot(A, x) - D*x,
        # avoiding the explicit creation and storage of the large matrix R.
        # A*x is np.dot(coefficient_matrix, x).
        # D_matrix*x is equivalent to D*x (element-wise) since D is the diagonal.
        off_diagonal_product = np.dot(coefficient_matrix, x) - D * x
        x = (constant_matrix.flatten() - off_diagonal_product) / D
