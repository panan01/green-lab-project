...
import numpy as np
# G5: Use float32 for less precision but faster computation and less memory usage.
...
def jacobi_iteration_method(
    coefficient_matrix: NDArray[float32],
    constant_matrix: NDArray[float32],
    init_val: list[float],
    iterations: int,
) -> list[float]:
    """
    ...
    Examples:
    >>> coefficient = np.array([[4, 1, 1], [1, 5, 2], [1, 2, 4]], dtype=np.float32)
    >>> constant = np.array([[2], [-6], [-4]], dtype=np.float32)
    ...
    # G5: Use float32 for all calculations.
    x = np.array(init_val, dtype=float32)

    # Decompose the coefficient matrix A into D (diagonal) and R (off-diagonal)
    # A = D + R
    D = np.diag(coefficient_matrix)
    R = coefficient_matrix - np.diag(np.diag(coefficient_matrix))
...
def strictly_diagonally_dominant(matrix: NDArray[float32]) -> bool:
    """
    Checks if the given matrix is strictly diagonally dominant.
    The condition is: |a_ii| > sum(|a_ij|) for all i where j != i.

    >>> table = np.array([[4, 1, 1], [1, 5, 2], [1, 2, 4]], dtype=np.float32)
    >>> strictly_diagonally_dominant(table)
    True

    >>> table = np.array([[4, 1, 1], [1, 5, 2], [3, 2, 4]], dtype=np.float32)
    >>> strictly_diagonally_dominant(table)
...
def generate_sdd_matrix(
    n: int,
) -> tuple[NDArray[float32], NDArray[float32], list[float]]:
    """
    ...
    # G5: Cast generated matrices to float32 to reduce memory and improve speed.
    ...
    np.fill_diagonal(A, row_sums + np.random.uniform(0.1, 1.0, n).astype(np.float32))
