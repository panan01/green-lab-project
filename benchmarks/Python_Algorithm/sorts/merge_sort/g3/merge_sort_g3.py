...
def merge_sort(collection: list) -> list:
    ...
    def merge(left: list, right: list) -> list:
        ...
        """
        # G3 Optimization: Use indices instead of pop(0) for efficient merging.
        # Pre-calculate and store loop end conditions.
        len_left = len(left)
        len_right = len(right)
        result = []
        i, j = 0, 0

        # Main loop for merging. Accessing by index is O(1).
        ...
        # After the main loop, one of the lists may have remaining elements.
        # Append the remaining slice, which is more efficient than a loop.
        if i < len_left:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "'large': A large dataset (2,00,000 elements) for performance/energy tests."
        )
    )
