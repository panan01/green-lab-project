...
import argparse
import string
# G8: Import lru_cache for memoization
...
def run_genetic_algorithm(target: str, genes: list[str], debug: bool = True) -> tuple[int, int, str]:
    ...
    # G8: The `evaluate` function is moved inside the main loop and decorated
    # with `lru_cache`. This caches its results, avoiding re-computation for
    # strings that have already been scored. The cache is specific to each
    # run of the algorithm.
    @lru_cache(maxsize=None)
    def evaluate(item: str) -> tuple[str, float]:
        """
        Evaluate how similar the item is with the target by just
        counting each char in the right position.
        The `target` variable is taken from the parent scope.
        >>> # This doctest can't be run directly as the function is now nested
        >>> # and depends on an outer scope 'target' variable.
        """
        score = len([g for position, g in enumerate(item) if g == target[position]])
    ...
    while True:
        ...
        # Evaluation phase using the memoized function
