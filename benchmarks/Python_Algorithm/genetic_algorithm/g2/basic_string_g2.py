...
def run_genetic_algorithm(target: str, genes: list[str], debug: bool = True) -> tuple[int, int, str]:
    ...
    num_elite = 0  # On the first run, there are no elite individuals
    ...
    while True:
        ...
        # G2 Optimization: Avoid re-sorting the elite, which is already sorted
        # from the previous generation. Instead, sort only the new members and
        # merge the two sorted lists.
        if num_elite > 0:
            # The population list consists of the elite from the last generation
            # followed by new children. The elite part is already sorted by score.
            elite_part = population_score[:num_elite]
            new_part = population_score[num_elite:]

            # Avoid re-sorting the already sorted elite part by only sorting the new part.
            new_part.sort(key=lambda x: x[1], reverse=True)

            # Merge the two sorted lists to get the final sorted population.
            # A manual merge is used here to handle descending order.
            full_sorted_list = []
            i, j = 0, 0
            while i < len(elite_part) and j < len(new_part):
                if elite_part[i][1] >= new_part[j][1]:
                    full_sorted_list.append(elite_part[i])
                    i += 1
                else:
                    full_sorted_list.append(new_part[j])
                    j += 1

            # Add any remaining elements from either list
            full_sorted_list.extend(elite_part[i:])
            full_sorted_list.extend(new_part[j:])
            population_score = full_sorted_list
        else:
            # For the first generation, the entire population is new and must be sorted.
        ...
        elite_count = int(N_POPULATION / 3)
        population_best = [item for item, score in population_score[:elite_count]]
        # For the next iteration's optimization, remember how many elite members we kept.
        num_elite = len(population_best)
        population.clear()
