...
def run_genetic_algorithm(target: str, genes: list[str], debug: bool = True) -> tuple[int, int, str]:
    ...
    # --- G7 OPTIMIZATION ---
    # Generate random starting population using a bulk operation.
    # random.choices() generates all necessary characters for a single member
    # at once, which is more efficient than calling random.choice() in a loop.
    population = [
        "".join(random.choices(genes, k=len(target)))
