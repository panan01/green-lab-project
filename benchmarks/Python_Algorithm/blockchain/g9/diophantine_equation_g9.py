...
def diophantine_all_soln(a: int, b: int, c: int, n: int = 2) -> None:
    ...
    48 -63
    """
    # G9 Optimization: Inlined the logic from `diophantine` to reduce function calls.
    # This avoids the call to `diophantine` and also prevents re-computing the GCD,
    # as `extended_gcd` already returns it.

    # The call to extended_gcd provides d=gcd(a,b) and the initial coefficients for the particular solution.
    ...
    # Calculate the particular solution (x0, y0) directly.
    # This logic was originally in the `diophantine` function.
    r = c // d
    x0 = r * x_part
    y0 = r * y_part

    # From the theorem, the offsets for x and y are (b/d) and (a/d).
    # We reuse 'd' from the extended_gcd call, avoiding another GCD calculation.
    q = b // d  # Offset for x
    p = a // d  # Offset for y
